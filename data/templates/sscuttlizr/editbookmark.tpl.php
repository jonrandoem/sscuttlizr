	...
	<div class="row">
		...
		<form action="<?php echo $formaction; ?>" method="post">
			...
			<div class="form-group">
					<?php echo T_('Address'); ?>
					<input class="form-control" type="text" id="address" name="address" size="75" maxlength="65535" value="<?php echo filter($row['bAddress'], 'xml'); ?>" onblur="useAddress(this)" />
					<i class="fa fa-chevron-up"></i>  <?php echo T_('Required'); ?>
			</div>
			<hr>
			<div class="form-group">
					<?php echo T_('Title'); ?>
					<input class="form-control" type="text" id="titleField" name="title" size="75" maxlength="255" value="<?php echo filter($row['bTitle'], 'xml'); ?>" onkeypress="this.style.backgroundImage = 'none';" />
					<i class="fa fa-chevron-up"></i>  <?php echo T_('Required'); ?>
			</div>
			<hr>
			<div class="form-group">
					...
					<textarea class="form-control" name="description" id="description" rows="5" cols="63" ><?php echo filter($row['bDescription'], 'xml'); ?></textarea>
					<i class="fa fa-chevron-up"></i>  <?php echo T_('You can use anchors to delimite attributes. for example: [publisher]blah[/publisher] '); ?>
			...
			<hr>
			<div class="form-group" id="privateNoteZone" <?php if(strlen($row['bPrivateNote'])==0):?>style="display:none"<?php endif; ?>>
					<?php echo T_('Private Note'); ?>
					<textarea class="form-control" name="privateNote" id="privateNote" rows="1" cols="63" ><?php echo filter($row['bPrivateNote'], 'xml'); ?></textarea>
					<i class="fa fa-chevron-up"></i>  <?php echo T_('Just visible by you and your contacts.'); ?>
					<hr>
			</div>
			
			<div class="form-group">
					<?php echo T_('Tags'); ?>
					<input class="form-control" type="text" id="tags" name="tags" size="75" value="<?php echo filter($row['tags'], 'xml'); ?>"/>
					<i class="fa fa-chevron-up"></i>  <?php echo T_('Comma-separated'); ?><br>
					<small><?php echo htmlspecialchars(T_('Note: use ">" to include one tag in another. e.g.: europe>france>paris'))?></small><br>
					<small><?php echo T_('Note: use "=" to make synonym two tags. e.g.: france=frenchcountry')?></small>
			</div>
			<hr>
			<div class="form-group">
					<?php echo T_('Privacy'); ?>
			...
			<hr>
			<div class="form-group">
							<input class="btn btn-default" type="submit" name="submitted" value="<?php echo $btnsubmit; ?>" />
							<input class="btn btn-default" type="button" name="cancel" value="<?php echo T_('Cancel') ?>" onclick="<?php echo $popup?'window.close();':'javascript: history.go(-1)'; ?>" />
							<?php
							if (isset($showdelete) && $showdelete) {
							?>
							<input class="btn btn-default" type="submit" name="delete" value="<?php echo T_('Delete Bookmark'); ?>" />
							<?php
							}
							if (isset($showdelete) && $showdelete) {
